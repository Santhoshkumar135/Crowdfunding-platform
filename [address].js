    ...
    const Request = async () => {
      ...
      setMydonations(MyAllDonations.map((e) => {
        return {
          ...
          timestamp: format(new Date(e.args.timestamp * 1000), "MM/dd/yyyy, hh:mm:ss a") // Format timestamp using date-fns
        }
      }));
...
export async function getStaticProps(context) {
  ...
  //const requiredAmountWei = ethers.utils.parseEther(requiredAmount.toString()).mul(ethers.BigNumber.from(10).pow(0));
